...
function syncProfilScreen(){
  ...
  if(S.customBudgetKcal>0){
    set('profile-manual-budget',S.customBudgetKcal);
    const prev=document.getElementById('manual-budget-preview');
    if(prev) prev.textContent=S.customBudgetKcal+' kcal/jour';
  }
...
function saveProfile(){
  ...
  // Budget calorique personnalisé (0 = utiliser le calcul TDEE-déficit)
  const manualBudgetVal=parseInt(document.getElementById('profile-manual-budget')?.value)||0;
  S.customBudgetKcal = manualBudgetVal > 0 ? manualBudgetVal : 0;
