...
function renderStepsWidget(){
  ...
  // Render stats chart if visible
  if(el('steps-stats-wrap')) renderStepsStats(currentStepsPeriod||'week');
  return kcalBurned;
}

// Steps statistics — multi-period
var currentStepsPeriod = 'week';

function setStepsPeriod(btn, period){
  currentStepsPeriod = period;
  ['sbtn-week','sbtn-month','sbtn-3month'].forEach(id=>{
    const el=document.getElementById(id);
    if(el)el.classList.remove('active');
  });
  if(btn)btn.classList.add('active');
  renderStepsStats(period);
}

function renderStepsStats(period){
  const wrap=document.getElementById('steps-stats-wrap');
  if(!wrap)return;
  const goal=S.stepsGoal||10000;
  const weight=parseFloat(S.profileWeight)||70;
  const now=new Date();
  const days=[];
  const nDays=period==='week'?7:period==='month'?30:90;
  ...
  const data=days.map(d=>{
    ...
    const steps=(S.stepsLog&&S.stepsLog[key])||0;
    return{date:d,key,steps};
  });
  const activeDays=data.filter(d=>d.steps>0);
  const totalSteps=data.reduce((t,d)=>t+d.steps,0);
  const avgSteps=activeDays.length?Math.round(totalSteps/activeDays.length):0;
  const totalKcal=Math.round(totalSteps*0.04*(weight/70));
  const totalKm=(totalSteps*0.00075).toFixed(1);
  const daysGoalReached=data.filter(d=>d.steps>=goal).length;
  ...
  if(period==='week'){
    bars=data.map(d=>{
      const h=Math.round((d.steps/maxSteps)*60);
      const isToday=d.key===fullDateKey(new Date());
      const clr=d.steps>=goal?'var(--green-glow)':d.steps>0?'var(--orange)':'var(--border)';
      const lbl=dayNames[d.date.getDay()];
      return`<div style="display:flex;flex-direction:column;align-items:center;gap:3px;flex:1">
        <div style="font-size:10px;color:var(--text-muted);font-weight:600">${d.steps>0?Math.round(d.steps/1000)+'k':''}</div>
        <div style="width:100%;max-width:32px;height:60px;background:var(--bg-tertiary);border-radius:5px 5px 0 0;display:flex;align-items:flex-end;overflow:hidden;border:1px solid var(--border)">
          <div style="width:100%;height:${h}px;background:${clr};border-radius:4px 4px 0 0;transition:height .5s"></div>
        </div>
        <div style="font-size:10px;color:${isToday?'var(--green-glow)':'var(--text-muted)'};font-weight:${isToday?'700':'500'}">${lbl}</div>
      </div>`;
    }).join('');
  } else {
    // For month/3month: group by week
    const weeks=[];
    let wk={days:[],total:0};
    data.forEach((d,i)=>{
      wk.days.push(d);wk.total+=d.steps;
      if(d.date.getDay()===0||i===data.length-1){
        weeks.push({...wk,avg:wk.days.filter(x=>x.steps>0).length?Math.round(wk.total/wk.days.filter(x=>x.steps>0).length):0,first:wk.days[0].date});
        wk={days:[],total:0};
      }
    });
    const maxW=Math.max(...weeks.map(w=>w.avg),goal,1);
    bars=weeks.map(w=>{
      const h=Math.round((w.avg/maxW)*60);
      const clr=w.avg>=goal?'var(--green-glow)':w.avg>0?'var(--orange)':'var(--border)';
      const lbl=w.first.getDate()+'/'+(w.first.getMonth()+1);
      return`<div style="display:flex;flex-direction:column;align-items:center;gap:3px;flex:1">
        <div style="font-size:9px;color:var(--text-muted)">${w.avg>0?Math.round(w.avg/1000)+'k':''}</div>
        <div style="width:100%;max-width:32px;height:60px;background:var(--bg-tertiary);border-radius:5px 5px 0 0;display:flex;align-items:flex-end;overflow:hidden;border:1px solid var(--border)">
          <div style="width:100%;height:${h}px;background:${clr};border-radius:4px 4px 0 0;transition:height .5s"></div>
        </div>
  ...
  wrap.innerHTML=`
    <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:7px;margin-bottom:10px">
      <div style="background:var(--bg-tertiary);border-radius:10px;padding:8px 10px;border:1px solid var(--border)">
        <div style="font-size:9px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.4px;margin-bottom:2px">Moy./jour</div>
        <div style="font-size:15px;font-weight:800;color:var(--green-glow)">${avgSteps.toLocaleString('fr-FR')}</div>
        <div style="font-size:10px;color:var(--text-muted)">pas</div>
      </div>
      <div style="background:var(--bg-tertiary);border-radius:10px;padding:8px 10px;border:1px solid var(--border)">
        <div style="font-size:9px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.4px;margin-bottom:2px">Distance</div>
        <div style="font-size:15px;font-weight:800;color:var(--orange)">${totalKm}</div>
        <div style="font-size:10px;color:var(--text-muted)">km</div>
      </div>
      <div style="background:var(--bg-tertiary);border-radius:10px;padding:8px 10px;border:1px solid var(--border)">
        <div style="font-size:9px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.4px;margin-bottom:2px">Objectif</div>
        <div style="font-size:15px;font-weight:800;color:var(--green-glow)">${daysGoalReached}</div>
        <div style="font-size:10px;color:var(--text-muted)">jours atteints</div>
      </div>
    </div>
    <div style="display:flex;gap:3px;align-items:flex-end;height:80px;padding-bottom:4px">
      ${bars}
    </div>
    <div style="display:flex;justify-content:space-between;margin-top:7px;font-size:11px;color:var(--text-muted)">
      <span>🔥 ${totalKcal.toLocaleString('fr-FR')} kcal brûlées</span>
      <span>🏃 ${totalSteps.toLocaleString('fr-FR')} pas total</span>
    </div>
  `;
}
...
function renderHomeHeader(){
  ...
  const imc=w&&h?Math.round(w/((h/100)**2)*10)/10:null;
  // Fix: update all IMC display elements
  if(imc){
    if(el('weight-imc-display'))el('weight-imc-display').textContent='IMC '+imc;
    if(el('home-imc-display'))el('home-imc-display').textContent='IMC '+imc;
  }
}
...
// ── Boot géré uniquement dans app.js ──
