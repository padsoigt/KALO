...
// Aliments jamais soumis aux ratios de cuisson (déjà prêts à consommer)
const NEVER_COOK_KEYWORDS = [
  'glace','ice cream','yaourt','yogurt','fromage','cheese','chocolat','beurre',
  'lait','milk','creme','cream','dessert','gateau','gâteau','biscuit','cookie',
  'confiture','miel','jus','juice','compote','conserve','surgele','surgelé',
  'sauce','vinaigrette','huile','oil','mayonnaise','ketchup','moutarde',
  'charcuterie','jambon','saucisse','pate','pâté','terrine','rillette',
  'chips','crackers','cereales','céréales','muesli','granola','pain','bread',
  'baguette','croissant','viennoiserie','brioche','gauffre','gaufre','crepe','crêpe',
  'noix','noisette','amande','cacahuete','cacahuète','pistache','abricot','fruit sec'
];
function isNeverCook(name){
  const n = name.toLowerCase();
  return NEVER_COOK_KEYWORDS.some(kw => n.includes(kw));
}
...
function estimateKcalVal(name,qty,unit){
  ...
  // 2. Valeurs nutritionnelles personnalisées : priorité absolue, jamais de ratio cuisson
  if(S.customNutrition){
    const normN = normStr(n);
    const customKey = Object.keys(S.customNutrition).find(k => {
      const kn = normStr(k);
      return normN === kn || normN.includes(kn) || kn.includes(normN.split(' ')[0]);
    });
    if(customKey){
      const cv = S.customNutrition[customKey];
      if(cv._perUnit){
        // Valeurs saisies par unité/portion → pas de calcul au poids, on multiplie par la quantité saisie
        const count = numQty || 1;
        const kcal  = Math.round((cv.kcal||0) * count);
        const gM    = Math.round((cv.g||0)    * count);
        const pM    = Math.round((cv.p||0)    * count);
        const lM    = Math.round((cv.l||0)    * count);
        const fM    = Math.round((cv.f||0)    * count);
        const detail= `${count} unité(s) × ${cv.kcal} kcal/unité = ${kcal} kcal (valeurs personnalisées)`;
        return{kcal,g:gM,p:pM,l:lM,f:fM,detail,rawNote:'📋 Valeurs par unité',coherenceWarning:null};
      }
      // Valeurs pour 100g → calcul proportionnel au poids
      const factor = grams / 100;
      const kcal  = Math.round((cv.kcal||0) * factor);
      const gM    = Math.round((cv.g||0)    * factor);
      const pM    = Math.round((cv.p||0)    * factor);
      const lM    = Math.round((cv.l||0)    * factor);
      const fM    = Math.round((cv.f||0)    * factor);
      const detail= `${grams}g × ${cv.kcal} kcal/100g = ${kcal} kcal (valeurs personnalisées)`;
      return{kcal,g:gM,p:pM,l:lM,f:fM,detail,rawNote:'📋 Valeurs personnalisées utilisées',coherenceWarning:null};
    }
  }

  const cooked=isExplicitlyCooked(name);
  // Pas de ratio cuisson pour les aliments prêts à consommer
  const cruEntry = isNeverCook(name) ? null : getCruCuitEntry(name);
  if(cruEntry&&!cooked){
    // Poids CRU saisi
    const factor=grams/100;
...
function saveMeal(){
  ...
  rows.forEach(row=>{
    ...
    if(iname){
      const est=estimateKcalVal(iname,qty,unit);
  ...
  S.meals[k].push({
    name, kcal, type:S.selMealType, ingredients,
    proteines:Math.round(totalP), glucides:Math.round(totalG), lipides:Math.round(totalL)
  });
...
function updateMeal(){
  ...
  const rows=[...document.querySelectorAll('#edit-ing-list .ing-row')];
  const ingredients=[];
  let computedKcal=0, totalP=0, totalG=0, totalL=0;
  rows.forEach(row=>{
    ...
    if(iname){
      const est=estimateKcalVal(iname,String(qty),unit);
      ingredients.push({name:iname,qty,unit});
      computedKcal+=est.kcal;
      totalP+=est.p||0; totalG+=est.g||0; totalL+=est.l||0;
    }
  });
  ...
  if(S.meals[editKey]&&S.meals[editKey][editIdx]!==undefined){
    S.meals[editKey][editIdx]={
      name,kcal,type:editMealType,ingredients,
      proteines:Math.round(totalP),glucides:Math.round(totalG),lipides:Math.round(totalL)
    };
  }
...
function addFromChat(name,kcal,type,ingredients,btn){
  ...
  // Calculer les macros depuis les ingrédients
  let totalP=0,totalG=0,totalL=0;
  ings.forEach(ing=>{
    const r=estimateKcalVal(ing.name,String(ing.qty||100),ing.unit||'g');
    totalP+=r.p||0; totalG+=r.g||0; totalL+=r.l||0;
  });
  // Si aucun ingrédient, estimer depuis kcal (ratio standard)
  if(!ings.length){
    totalP=Math.round(kcal*0.30/4);
    totalG=Math.round(kcal*0.45/4);
    totalL=Math.round(kcal*0.25/9);
  }
  const todayKey=fullDateKey(new Date());
  if(!S.meals[todayKey]) S.meals[todayKey]=[];
  S.meals[todayKey].push({
    name,kcal,type,ingredients:ings,
    proteines:Math.round(totalP),glucides:Math.round(totalG),lipides:Math.round(totalL)
  });
  save(); updateChatKcal();
  if(btn){ btn.textContent='✓ Ajouté à l\'agenda !'; btn.style.background='#3d8a3c'; btn.disabled=true; }
}
...
function calcTDEE(){
  ...
  // Budget personnalisé : si défini et > 0, il prime sur le calcul TDEE-déficit
  const manualBudget=parseInt(document.getElementById('profile-manual-budget')?.value)||S.customBudgetKcal||0;
  const budget=manualBudget>0 ? manualBudget : Math.max(1200,tdee-deficit);
  return{tdee,bmr,deficit,budget,manualBudget};
}
...
function saveManualMeal(){
  ...
  const proteines=parseFloat(document.getElementById('manual-p')?.value)||0;
  const glucides =parseFloat(document.getElementById('manual-g')?.value)||0;
  const lipides  =parseFloat(document.getElementById('manual-l')?.value)||0;
  // If macros not entered, estimate from kcal split (P30% G45% L25%)
  const p=proteines||Math.round(kcal*0.30/4);
  const g=glucides ||Math.round(kcal*0.45/4);
  const l=lipides  ||Math.round(kcal*0.25/9);
  const k=fullDateKey(S.selDate);
  if(!S.meals[k]) S.meals[k]=[];
  S.meals[k].push({name,kcal,type:S.selMealType,ingredients:[],proteines:p,glucides:g,lipides:l});
  save(); closeMealModal(); renderAgenda();
}
