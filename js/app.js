...
function initApp(){
  ...
  try{ renderStepsStats('week'); }catch(e){ console.warn('[init steps stats]',e); }
...
// estimation listeners attached in initApp()
...
(async function bootKalo(){
  // 1. Charger les données persistées EN PREMIER
  try {
    await loadIDB();
  } catch(e) {
  ...
  // 2. Charger les fichiers nutrition (ne bloque pas l'affichage)
  try {
  ...
  // 3. Forcer l'onboarding désactivé
  S.onboardingDone = true;
  const obScreen = document.getElementById('onboarding-screen');
  if(obScreen) obScreen.style.display = 'none';

  // 4. Attendre que le navigateur ait terminé son premier rendu
  //    avant d'appeler initApp() — corrige la page vierge au démarrage
  requestAnimationFrame(()=>{
    requestAnimationFrame(()=>{
      initApp();
      // 5. Second rendu différé pour s'assurer que tout est bien affiché
      setTimeout(()=>{
        try{ renderCalorieRing();   }catch(e){}
        try{ renderWeightChart();   }catch(e){}
        try{ renderStepsWidget();   }catch(e){}
        try{ renderStepsStats(currentStepsPeriod||'week'); }catch(e){}
        try{ renderHomeHeader();    }catch(e){}
        try{ renderHomeAnalytics(); }catch(e){}
        try{ renderWaterWidget();   }catch(e){}
      }, 100);
    });
  });
})();
