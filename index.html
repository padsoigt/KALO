...
// Référence nutritionnelle courante : '100g' ou 'unit'
let _frigoNutrRef = '100g';

function setNutrRef(ref){
  _frigoNutrRef = ref;
  const is100g = ref === '100g';
  // Boutons toggle
  document.getElementById('nutr-ref-100g')?.classList.toggle('active', is100g);
  document.getElementById('nutr-ref-unit')?.classList.toggle('active', !is100g);
  // Label descriptif
  const lbl = document.getElementById('nutr-ref-label');
  if(lbl) lbl.textContent = is100g
    ? 'Valeurs pour 100g — données du tableau nutritionnel'
    : 'Valeurs par unité / portion (ex : 1 yaourt, 1 barre)';
  // Labels des champs
  const suf = is100g ? '/100g' : '/unité';
  const upd = (id, base) => { const el=document.getElementById(id); if(el) el.textContent = base+' ('+suf+')'; };
  upd('lbl-frigo-kcal', 'Calories (kcal)');
  upd('lbl-frigo-prot', 'Protéines (g)');
  upd('lbl-frigo-gluc', 'Glucides (g)');
  upd('lbl-frigo-lip',  'Lipides (g)');
  upd('lbl-frigo-fib',  'Fibres (g)');
}
...
function openFrigoModal(){
  ...
  // Remettre le toggle à 100g
  setNutrRef('100g');
  document.getElementById('frigo-modal').classList.add('open');
  // Focus sur le champ nom après animation
  setTimeout(() => document.getElementById('new-ingredient')?.focus(), 320);
}
...
function addIngredient(){
  const val = document.getElementById('new-ingredient').value.trim();
  if(!val) return;
  ...
  const kcalRaw = parseFloat(document.getElementById('frigo-kcal')?.value);
  if(kcalRaw > 0){
    if(!S.customNutrition) S.customNutrition = {};
    const entry = {
      kcal: kcalRaw,
      p:    parseFloat(document.getElementById('frigo-prot')?.value) || 0,
    ...
    // Si les valeurs sont par unité, on les conserve telles quelles avec un flag
    if(_frigoNutrRef === 'unit'){
      entry._perUnit = true; // flag pour estimateKcalVal
    }
    S.customNutrition[val.toLowerCase()] = entry;
  }
